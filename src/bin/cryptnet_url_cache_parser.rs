...
fn output_data_csv(parsed_data: &CryptnetURLCacheParser) -> io::Result<String> {
    if parsed_data.sha256.is_none() {
        return Ok(format!(
            "\"{}\",\"{}\",\"{}\",\"{}\",\"{}\",\"{}\"\r\n",
            parsed_data.last_download_time,
            parsed_data.last_modification_time_header,
            parsed_data.url,
            parsed_data.file_size,
            parsed_data.e_tag,
            parsed_data.full_path
        ));
    } else {
        return Ok(format!(
            "\"{}\",\"{}\",\"{}\",\"{}\",\"{}\",\"{}\",\"{}\"\r\n",
            ...
            parsed_data.e_tag,
            parsed_data.full_path,
            parsed_data.sha256.as_ref().unwrap()
        ));
    }
}
        ...
        match output_format {
            OutputFormat::CSV => {
                if use_content == Some(true) {
                    output.write("\"LastDownloadTime\",\"LastModificationTimeHeader\",\"URL\",\"FileSize (Bytes)\",\"ETag\",\"FullPath\",\"SHA256\"\r\n".as_bytes()).expect("Error Writing Data !");
                } else {
                    output
                        .write(
                            "\"LastDownloadTime\",\"LastModificationTimeHeader\",\"URL\",\"FileSize (Bytes)\",\"ETag\",\"FullPath\"\r\n"
                                .as_bytes(),
                        )
